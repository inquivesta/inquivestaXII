...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!team_name || !team_leader_name || !team_leader_email || !team_leader_phone || !institution || !category) {
      return NextResponse.json({ error: 'All team leader fields are required' }, { status: 400 })
    }

    // Validate partner details
    if (!partner || !partner.name || !partner.email || !partner.phone) {
      return NextResponse.json({ error: 'Partner details are required' }, { status: 400 })
    }

    // Check for duplicate team leader email
    const { data: existingLeaderRegistration } = await supabaseAdmin
    ...
    if (existingLeaderRegistration) {
      return NextResponse.json(
        { error: 'This email is already registered for Table Tennis Doubles' },
        { status: 400 }
      )
    }

    // Also check if partner email is already registered as team leader
    const { data: existingPartnerRegistration } = await supabaseAdmin
      .from(EVENT_CONFIG.tableName)
      .select('id')
      .eq('team_leader_email', partner.email.toLowerCase())
      .single()

    if (existingPartnerRegistration) {
      return NextResponse.json(
        { error: 'Partner email is already registered for Table Tennis Doubles' },
        { status: 400 }
      )
    }
    ...
    const categoryLabel = category === 'mens' ? "Men's Doubles" : "Mixed Doubles"
    const isIISERK = team_leader_email.toLowerCase().endsWith('@iiserkol.ac.in')
    ...
    try {
      await resend.emails.send({
        ...
        html: generateTTDoublesEmailHTML({
          teamName: team_name,
          leaderName: team_leader_name,
          leaderPhone: team_leader_phone,
          institution,
...
function generateTTDoublesEmailHTML(data: TTDoublesEmailData): string {
...
<head>
  ...
  <title>Team Registration Confirmed - Table Tennis Doubles</title>
</head>
          ...
          <tr>
            <td style="padding: 30px; font-family: 'Courier New', monospace;">
              <p style="color: #cccccc; font-size: 14px; line-height: 1.6; margin: 0 0 20px 0;">
                ...
                Category: <span style="background-color: ${themeColor}; color: white; padding: 0 4px; font-size: 12px;">${data.category}</span>
              </p>
              
              <p style="color: #cccccc; font-size: 14px; line-height: 1.6; margin: 0 0 30px 0;">
                Your team registration for <strong style="color: #ffffff; border-bottom: 1px dotted ${themeColor};">Table Tennis Doubles</strong> has been successfully confirmed!
              </p>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="border: 1px solid #333; margin-bottom: 30px;">
                ...
                <tr>
                  <td colspan="2" style="padding: 15px; border-bottom: 1px solid #333;">
                    <p style="margin: 0; font-size: 10px; color: #888; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 5px;">TEAM NAME</p>
                    <p style="margin: 0; font-size: 13px; color: #ccc;">${data.teamName}</p>
                  </td>
                </tr>
                <tr>
                  <td colspan="2" style="padding: 15px; border-bottom: 1px solid #333;">
                    <p style="margin: 0; font-size: 10px; color: #888; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 8px;">PLAYER 1 (TEAM LEADER)</p>
                    <p style="margin: 0; font-size: 13px; color: #ccc;">
                      <strong>${data.leaderName}</strong> (${data.leaderPhone})
                    </p>
                  </td>
                </tr>
                <tr>
                  <td colspan="2" style="padding: 15px; border-bottom: 1px solid #333;">
                    <p style="margin: 0; font-size: 10px; color: #888; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 8px;">PLAYER 2 (PARTNER)</p>
                    <p style="margin: 0; font-size: 13px; color: #ccc;">
                      <strong>${data.partner.name}</strong> (${data.partner.phone})
                    </p>
                  </td>
                </tr>
                <tr>
                ...
                <tr>
                  <td style="padding: 15px;">
                    ...
                    <ul style="color: #cccccc; margin: 0; padding-left: 20px; font-size: 12px; line-height: 1.6; list-style-type: square;">
                      <li>Both team members must be present at the venue.</li>
                      <li>Arrive at least 30 minutes before your scheduled match.</li>
                      <li>Bring valid ID cards for both players.</li>
                      <li>Wear appropriate sports attire and non-marking shoes.</li>
                      <li>Bring your own rackets (tables and balls will be provided).</li>
                    </ul>
                  </td>
                </tr>
              </table>
          ...
          <tr>
            <td style="background-color: #000000; padding: 20px; text-align: center; border-top: 2px solid ${themeColor};">
              <p style="color: ${themeColor}; margin: 0 0 10px 0; font-size: 12px; letter-spacing: 2px; text-transform: uppercase; font-family: monospace;">
                INQUIVESTA XII - TABLE TENNIS DOUBLES
              </p>
