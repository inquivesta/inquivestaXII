...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!participant_name || !participant_email || !participant_phone || !institution || !category) {
    ...
    if (existingRegistration) {
      return NextResponse.json(
        { error: 'This email is already registered for Table Tennis Singles' },
        { status: 400 }
      )
    }
    ...
    const categoryLabel = category === 'mens' ? "Men's Singles" : "Women's Singles"
    const isIISERK = participant_email.toLowerCase().endsWith('@iiserkol.ac.in')
    ...
    try {
      await resend.emails.send({
        ...
        subject: `Registration Confirmed - ${EVENT_CONFIG.name} | Inquivesta XII`,
        html: generateTTSinglesEmailHTML({
...
function generateTTSinglesEmailHTML(data: TTSinglesEmailData): string {
  const currentYear = new Date().getFullYear()
  const themeColor = '#22C55E' // Green
...
<head>
  ...
  <title>Registration Confirmed - Table Tennis Singles</title>
</head>
          ...
          <tr>
            <td style="padding: 30px; font-family: 'Courier New', monospace;">
              <p style="color: #cccccc; font-size: 14px; line-height: 1.6; margin: 0 0 20px 0;">
                <span style="color: ${themeColor};">&gt; PLAYER DETAILS</span><br>
              ...
              <p style="color: #cccccc; font-size: 14px; line-height: 1.6; margin: 0 0 30px 0;">
                Your registration for <strong style="color: #ffffff; border-bottom: 1px dotted ${themeColor};">Table Tennis Singles</strong> has been successfully confirmed!
              </p>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="border: 1px solid #333; margin-bottom: 30px;">
                ...
                <tr>
                  <td style="padding: 15px; border-bottom: 1px solid #333; vertical-align: top;">
                    ...
                    <p style="margin: 0; font-size: 13px; color: #ccc;">${data.participantName}</p>
                  </td>
                  <td style="padding: 15px; border-bottom: 1px solid #333; vertical-align: top;">
                    <p style="margin: 0; font-size: 10px; color: #888; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 5px;">CATEGORY</p>
                    <p style="margin: 0; font-size: 13px; color: ${themeColor};">${data.category}</p>
                  </td>
                </tr>
                <tr>
                ...
                <tr>
                  <td style="padding: 15px;">
                    ...
                    <ul style="color: #cccccc; margin: 0; padding-left: 20px; font-size: 12px; line-height: 1.6; list-style-type: square;">
                      ...
                      <li>Bring your own racket (tables and balls will be provided).</li>
                    </ul>
                  </td>
                </tr>
              </table>
          ...
          <tr>
            <td style="background-color: #000000; padding: 20px; text-align: center; border-top: 2px solid ${themeColor};">
              <p style="color: ${themeColor}; margin: 0 0 10px 0; font-size: 12px; letter-spacing: 2px; text-transform: uppercase; font-family: monospace;">
                INQUIVESTA XII - TABLE TENNIS SINGLES
              </p>
