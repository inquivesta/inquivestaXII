...
import { ArrowLeft, Plus, Trash2, Users, CheckCircle, Compass } from "lucide-react"
...
export default function LostRegistrationPage() {
  const [selectedQR, setSelectedQR] = useState<number>(0)
  const [isSubmitted, setIsSubmitted] = useState(false)
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [errorMessage, setErrorMessage] = useState("")
  const [registrationId, setRegistrationId] = useState("")
  const [isIISERK, setIsIISERK] = useState(false)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validMembers = teamMembers.filter(m => m.name.trim() && m.email.trim() && m.phone.trim())
        ...
        <main className="container mx-auto px-4 py-20">
          <FadeIn>
            <div className="max-w-2xl mx-auto text-center">
              <div className="mb-8">
                <div className="w-24 h-24 mx-auto mb-6 bg-gradient-to-r from-[#2E8B57] to-[#228B22] rounded-full flex items-center justify-center">
                  <CheckCircle className="w-12 h-12 text-white" />
                </div>
                <HyperText
                  className="text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-[#2E8B57] to-[#228B22] bg-clip-text text-transparent font-futura tracking-wide"
                  text="REGISTERED!"
                />
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-3 text-left">
                    ...
                    <div className="flex justify-between py-2 border-b border-[#D2B997]/20">
                      ...
                      <span className="text-white font-depixel-body">{isIISERK ? "IISER Student" : "External"}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b border-[#D2B997]/20">
                  ...
                  <div className="bg-[#1A1A1A]/50 p-4 rounded-lg text-sm text-[#D2B997]/80 font-depixel-small space-y-2">
                    ...
                    <p>🧭 Get ready to navigate the unknown!</p>
                  </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center w-full">
                <Link href="/events" className="w-full sm:w-auto">
                  <Button className="w-full sm:w-auto bg-gradient-to-r from-[#2E8B57] to-[#228B22] hover:from-[#267349] hover:to-[#1E7A1E] text-white font-depixel-body px-6 sm:px-8 py-5 sm:py-6 text-sm sm:text-base">
                    Explore More Events
                  </Button>
                </Link>
        ...
        <main className="container mx-auto px-4 py-20">
          <FadeIn>
            <div className="text-center mb-12">
              <HyperText
                className="text-4xl md:text-6xl font-bold mb-6 bg-gradient-to-r from-[#2E8B57] to-[#228B22] bg-clip-text text-transparent font-futura tracking-wide"
                text="L.O.S.T"
              />
              <p className="text-[#D2B997] text-lg font-depixel-body">
                The Ultimate Treasure Hunt Adventure
              </p>
          ...
          <FadeIn delay={0.1}>
            <div className="max-w-4xl mx-auto mb-8">
              <div className="bg-gradient-to-r from-[#2E8B57]/10 to-[#228B22]/10 border border-[#2E8B57]/30 rounded-lg p-4">
                <h3 className="text-[#2E8B57] font-futura tracking-wide text-lg mb-2">💰 Registration Fees</h3>
                <div className="flex flex-wrap gap-4 text-sm font-depixel-small">
                  <span className="bg-[#1A1A1A]/50 px-3 py-1 rounded">
                    IISER Students: <span className="text-[#F4D03F]">₹120</span> per team
                    <span className="text-[#2E8B57] text-xs ml-1">(Use @iiserkol.ac.in email)</span>
                  </span>
                  <span className="bg-[#1A1A1A]/50 px-3 py-1 rounded">External: <span className="text-[#F4D03F]">₹160</span> per team</span>
                </div>
              </div>
            </div>
          </FadeIn>
          ...
          <form onSubmit={handleSubmit} className="max-w-4xl mx-auto space-y-8">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      ...
                      {formData.team_leader_email.toLowerCase().endsWith("@iiserkol.ac.in") ? (
                        <p className="text-green-400 text-xs mt-1">✓ IISER Kolkata email detected - Fee: ₹120</p>
                      ) : formData.team_leader_email && (
                        <p className="text-[#D2B997]/60 text-xs mt-1">External participant - Fee: ₹160</p>
                      )}
                    </div>
                    <div>
                    ...
                    teamMembers.map((member, index) => (
                      <div key={index} className="bg-[#1A1A1A]/50 p-4 rounded-lg border border-[#D2B997]/20">
                        <div className="flex items-center justify-between mb-4">
                          <span className="text-[#2E8B57] font-depixel-small">Member {index + 2}</span>
                          <Button
                            type="button"
                ...
                <CardContent>
                  <div className="flex flex-col md:flex-row gap-6">
                    ...
                    <div className="flex-1 space-y-4">
                      <div className="bg-gradient-to-r from-[#2E8B57]/10 to-[#228B22]/10 rounded-lg border border-[#D2B997]/20 p-4">
                        ...
                        <p className="text-[#D2B997]/60 font-depixel-small text-xs mt-2">
                          {isIISERK ? "IISER student rate applied" : "External participant rate"}
                        </p>
                      </div>
            ...
            <FadeIn delay={0.5}>
              ...
              <Button
                ...
                className="w-full bg-gradient-to-r from-[#2E8B57] to-[#228B22] hover:from-[#267349] hover:to-[#1E7A1E] text-white font-depixel-body py-6 text-base disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSubmitting ? (
                  <>
