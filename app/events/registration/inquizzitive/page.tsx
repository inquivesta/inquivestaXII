...
export default function InquizzitiveRegistrationPage() {
  ...
  const addTeamMember = () => {
    if (teamMembers.length < EVENT_CONFIG.maxTeamSize - 1) {
      setTeamMembers([...teamMembers, { name: "", college: "" }])
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validMembers = teamMembers.filter(m => m.name.trim() && m.college.trim())
    
    // Check if members have incomplete data
    const incompleteMembers = teamMembers.filter(m => (m.name.trim() && !m.college.trim()) || (!m.name.trim() && m.college.trim()))
    if (incompleteMembers.length > 0) {
      setErrorMessage("Please fill in both name and college for all team members")
      return
    }
    ...
    try {
      const response = await fetch("/api/events/register", {
        ...
        body: JSON.stringify({
          ...
          team_leader_college: formData.team_leader_college,
          team_size: 1 + validMembers.length,
          ...
          <FadeIn>
            <div className="text-center mb-12">
              ...
              <div className="mt-4 inline-flex items-center gap-2 bg-[#F4D03F]/10 border border-[#F4D03F]/30 px-4 py-2 rounded-full">
                ...
                <span className="text-[#F4D03F] font-depixel-small">Team Size: {EVENT_CONFIG.minTeamSize}-{EVENT_CONFIG.maxTeamSize} members (Solo allowed)</span>
              </div>
            </div>
          </FadeIn>
            ...
            <FadeIn delay={0.2}>
              <Card className="bg-[#2A2A2A]/50 border-[#D2B997]/30">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="team_leader_college" className="text-[#D2B997] font-depixel-small">College *</Label>
                      <Input
                        id="team_leader_college"
                        required
                        value={formData.team_leader_college}
                        onChange={(e) => setFormData({ ...formData, team_leader_college: e.target.value })}
                        className="bg-[#1A1A1A] border-[#D2B997]/30 text-white font-depixel-small"
                        placeholder="Your college name"
                      />
                    </div>
                    <div>
                      <Label htmlFor="team_leader_email" className="text-[#D2B997] font-depixel-small">College Email *</Label>
                      <Input
                  ...
                  <div>
                    <Label htmlFor="team_leader_phone" className="text-[#D2B997] font-depixel-small">WhatsApp Number *</Label>
                    <Input
                    ...
                    teamMembers.map((member, index) => (
                      <div key={index} className="bg-[#1A1A1A]/50 p-4 rounded-lg border border-[#D2B997]/20">
                        ...
                        <div className="grid md:grid-cols-2 gap-4">
                          ...
                          <div>
                            <Label className="text-[#D2B997] font-depixel-small text-xs">College *</Label>
                            <Input
