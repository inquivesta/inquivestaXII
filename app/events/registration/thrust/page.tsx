...
import { ArrowLeft, Plus, Trash2, Users, CheckCircle, Rocket } from "lucide-react"
...
export default function ThrustRegistrationPage() {
  const [selectedQR, setSelectedQR] = useState<number>(0)
  const [isSubmitted, setIsSubmitted] = useState(false)
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [errorMessage, setErrorMessage] = useState("")
  const [registrationId, setRegistrationId] = useState("")
  const [willBringBottle, setWillBringBottle] = useState(false)
  const [isIISERKOrSchool, setIsIISERKOrSchool] = useState(false)
  ...
  // Auto-detect IISER email
  useEffect(() => {
    if (formData.team_leader_email.toLowerCase().endsWith("@iiserkol.ac.in")) {
      setIsIISERKOrSchool(true)
    }
  }, [formData.team_leader_email])

  const getFee = () => isIISERKOrSchool ? 30 : 50
  ...
  const addTeamMember = () => {
    if (teamMembers.length < EVENT_CONFIG.maxTeamSize - 1) {
      setTeamMembers([...teamMembers, { name: "", institute: "", phone: "", email: "" }])
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validMembers = teamMembers.filter(m => m.name.trim() && m.institute.trim() && m.phone.trim() && m.email.trim())
    const totalSize = 1 + validMembers.length

    if (totalSize > EVENT_CONFIG.maxTeamSize) {
      setErrorMessage(`Team cannot have more than ${EVENT_CONFIG.maxTeamSize} members`)
      return
    }

    // Check if members have incomplete data
    const incompleteMembers = teamMembers.filter(m => 
      (m.name.trim() || m.institute.trim() || m.phone.trim() || m.email.trim()) && 
      !(m.name.trim() && m.institute.trim() && m.phone.trim() && m.email.trim())
    )
    if (incompleteMembers.length > 0) {
      setErrorMessage("Please fill in all fields for each team member")
      return
    }
    ...
    try {
      const response = await fetch("/api/events/register", {
        ...
        body: JSON.stringify({
          ...
          will_bring_bottle: willBringBottle,
          is_iiserk_or_school: isIISERKOrSchool,
          amount_paid: getFee(),
        ...
        <main className="container mx-auto px-4 py-20">
          <FadeIn>
            <div className="max-w-2xl mx-auto text-center">
              <div className="mb-8">
                <div className="w-24 h-24 mx-auto mb-6 bg-gradient-to-r from-[#FF6B6B] to-[#FF8E53] rounded-full flex items-center justify-center">
                  <CheckCircle className="w-12 h-12 text-[#1A1A1A]" />
                </div>
                <HyperText
                  className="text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-[#FF6B6B] to-[#FF8E53] bg-clip-text text-transparent font-futura tracking-wide"
                  text="REGISTERED!"
                />
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-3 text-left">
                    ...
                    <div className="flex justify-between py-2 border-b border-[#D2B997]/20">
                      ...
                      <span className="text-white font-depixel-body">{isIISERKOrSchool ? "IISER/School" : "External"}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b border-[#D2B997]/20">
                      <span className="text-[#D2B997]/80 font-depixel-small">Bringing Bottle:</span>
                      <span className="text-white font-depixel-body">{willBringBottle ? "Yes" : "No"}</span>
                    </div>
                    <div className="flex justify-between py-2 border-b border-[#D2B997]/20">
                      <span className="text-[#D2B997]/80 font-depixel-small">Amount Paid:</span>
                      <span className="text-[#F4D03F] font-futura tracking-wide text-xl">₹{getFee()}</span>
                    </div>
                  </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center w-full">
                <Link href="/events" className="w-full sm:w-auto">
                  <Button className="w-full sm:w-auto bg-gradient-to-r from-[#FF6B6B] to-[#FF8E53] hover:from-[#E55A5A] hover:to-[#E57D42] text-[#1A1A1A] font-depixel-body px-6 sm:px-8 py-5 sm:py-6 text-sm sm:text-base">
                    Explore More Events
                  </Button>
                </Link>
        ...
        <main className="container mx-auto px-4 py-20">
          <FadeIn>
            <div className="text-center mb-12">
              <HyperText
                className="text-4xl md:text-6xl font-bold mb-6 bg-gradient-to-r from-[#FF6B6B] to-[#FF8E53] bg-clip-text text-transparent font-futura tracking-wide"
                text="THRUST"
              />
              <p className="text-[#D2B997] text-lg font-depixel-body">
                Water Rocket Competition - Launch your way to victory!
              </p>
              <div className="mt-4 inline-flex items-center gap-2 bg-[#F4D03F]/10 border border-[#F4D03F]/30 px-4 py-2 rounded-full">
                ...
                <span className="text-[#F4D03F] font-depixel-small">Team Size: Up to {EVENT_CONFIG.maxTeamSize} members</span>
              </div>
            </div>
          </FadeIn>
          ...
          <FadeIn delay={0.1}>
            <div className="max-w-4xl mx-auto mb-8">
              <div className="bg-gradient-to-r from-[#FF6B6B]/10 to-[#FF8E53]/10 border border-[#FF6B6B]/30 rounded-lg p-4">
                <h3 className="text-[#FF6B6B] font-futura tracking-wide text-lg mb-2">💰 Registration Fees</h3>
                <div className="flex flex-wrap gap-4 text-sm font-depixel-small">
                  <span className="bg-[#1A1A1A]/50 px-3 py-1 rounded">IISER Students: <span className="text-[#F4D03F]">₹30</span></span>
                  <span className="bg-[#1A1A1A]/50 px-3 py-1 rounded">School Students (upto Class 10): <span className="text-[#F4D03F]">₹30</span></span>
                  <span className="bg-[#1A1A1A]/50 px-3 py-1 rounded">Others: <span className="text-[#F4D03F]">₹50</span></span>
                </div>
              </div>
            </div>
          </FadeIn>
          ...
          <form onSubmit={handleSubmit} className="max-w-4xl mx-auto space-y-8">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      ...
                      />
                      {formData.team_leader_email.toLowerCase().endsWith("@iiserkol.ac.in") && (
                        <p className="text-green-400 text-xs mt-1">✓ IISER Kolkata email detected</p>
                      )}
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      className="bg-[#1A1A1A] border-[#D2B997]/30 text-white font-depixel-small"
                      placeholder="Your institute/college/school name"
                    />
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="bg-[#2A2A2A]/50 border-[#D2B997]/30">
                <CardHeader>
                  <CardTitle className="text-2xl text-[#D2B997] font-futura tracking-wide flex items-center justify-between">
                    <span>Other Team Members (Optional)</span>
                ...
                <CardContent className="space-y-4">
                  {teamMembers.length === 0 ? (
                    <p className="text-[#D2B997]/60 font-depixel-small text-center py-4">
                      No additional members added. You can participate solo or add team members.
                    </p>
                  ) : (
                    teamMembers.map((member, index) => (
                      <div key={index} className="bg-[#1A1A1A]/50 p-4 rounded-lg border border-[#D2B997]/20">
                        <div className="flex items-center justify-between mb-4">
                          <span className="text-[#FF6B6B] font-depixel-small">Member {index + 2}</span>
                          <Button
                            type="button"
            ...
            <FadeIn delay={0.35}>
              <Card className="bg-[#2A2A2A]/50 border-[#D2B997]/30">
                <CardHeader>
                  <CardTitle className="text-2xl text-[#D2B997] font-futura tracking-wide">Additional Options</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="flex items-center space-x-3 bg-[#1A1A1A]/50 p-4 rounded-lg">
                    <Checkbox
                      id="willBringBottle"
                      checked={willBringBottle}
                      onCheckedChange={(checked) => setWillBringBottle(checked as boolean)}
                      className="border-[#D2B997]/50 data-[state=checked]:bg-[#FF6B6B] data-[state=checked]:border-[#FF6B6B]"
                    />
                    <Label htmlFor="willBringBottle" className="text-white font-depixel-small cursor-pointer">
                      I will bring my own 1L bottle 🍼
                    </Label>
                  </div>
                  ...
                  <div className="flex items-center space-x-3 bg-[#1A1A1A]/50 p-4 rounded-lg">
                    <Checkbox
                      id="isIISERKOrSchool"
                      checked={isIISERKOrSchool}
                      onCheckedChange={(checked) => setIsIISERKOrSchool(checked as boolean)}
                      className="border-[#D2B997]/50 data-[state=checked]:bg-[#F4D03F] data-[state=checked]:border-[#F4D03F]"
                    />
                    <div>
                      <Label htmlFor="isIISERKOrSchool" className="text-white font-depixel-small cursor-pointer">
                        I am an IISER Kolkata student OR School student (upto Class 10)
                      </Label>
                      <p className="text-[#D2B997]/60 text-xs mt-1">
                        {isIISERKOrSchool ? "Fee: ₹30" : "Fee: ₹50"} per team
                      </p>
                    </div>
                  </div>
                </CardContent>
                  ...
                  <div className="flex flex-col md:flex-row gap-6">
                    <div className="flex-1">
                      ...
                      <p className="text-center text-[#D2B997]/80 font-depixel-small text-sm mt-4">
                        Scan QR code to pay <span className="text-[#F4D03F] font-bold">₹{getFee()}</span>
                      </p>
                    ...
                    <div className="flex-1 space-y-4">
                      <div className="bg-gradient-to-r from-[#FF6B6B]/10 to-[#FF8E53]/10 rounded-lg border border-[#D2B997]/20 p-4">
                        <div className="flex justify-between items-center">
                          <span className="text-white font-depixel-body">Registration Fee:</span>
                          <span className="text-[#F4D03F] font-futura tracking-wide text-3xl">₹{getFee()}</span>
                        </div>
                        <p className="text-[#D2B997]/60 font-depixel-small text-xs mt-2">
                          {isIISERKOrSchool ? "IISER/School rate applied" : "Standard rate"}
                        </p>
                      </div>
            ...
            <FadeIn delay={0.5}>
              ...
              <Button
                ...
                className="w-full bg-gradient-to-r from-[#FF6B6B] to-[#FF8E53] hover:from-[#E55A5A] hover:to-[#E57D42] text-[#1A1A1A] font-depixel-body py-6 text-base disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSubmitting ? (
                  <>
